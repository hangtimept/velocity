...
(() => {
  ...
  function enableColorSample() {
    ...
    $('#tracker').classList.add('sampling');
  ...
  function exitColorSample() {
    state.samplingColor = false;
    $('#color-mode').classList.add('hidden');
    $('#tracker').classList.remove('sampling');
  ...
  function sampleColorAt(clientX, clientY) {
    ...
    // Map tap through object-fit: cover
    const wrap = document.querySelector('.camera-wrap');
    ...
    if (x < 0 || y < 0 || x >= video.videoWidth || y >= video.videoHeight) {
      console.warn('Tap outside video area', x, y);
      return;
    }
    ...
    state.hueTol = 30;
    state.satMin = Math.max(0.12, state.targetHSV.s * 0.3);
    state.valMin = Math.max(0.1, state.targetHSV.v * 0.2);
  ...
  $('#btn-cancel-sample').addEventListener('click', exitColorSample);

  // Taps on the camera area (overlay + wrap) sample the color
  function onSampleTap(e) {
    if (!state.samplingColor) return;
    e.preventDefault();
    e.stopPropagation();
    const t = e.touches ? e.touches[0] : e;
    if (t) sampleColorAt(t.clientX, t.clientY);
  }
  overlay.addEventListener('click', onSampleTap);
  overlay.addEventListener('touchstart', onSampleTap, { passive: false });
  const cameraWrap = document.querySelector('.camera-wrap');
  if (cameraWrap) {
    cameraWrap.addEventListener('click', onSampleTap);
    cameraWrap.addEventListener('touchstart', onSampleTap, { passive: false });
  }
